    ...
    def post(self, request):
        ...
        if not client.telegram_chat_id:
            return Response(
                {
                    "ok": False,
                    "detail": "Telegram не подключен. Сначала подключите Telegram в разделе «Интеграции».",
                },
        ...
        try:
            ...
            logger.exception("Failed to send manual report to Telegram client_id=%s", client.id)
            settings_obj.last_error = str(exc)
            settings_obj.save(update_fields=["last_error", "updated_at"])
            return Response(
                {"ok": False, "detail": "Не удалось отправить PDF-отчёт в Telegram."},
        ...
        return Response({"ok": True, "detail": "PDF-отчёт отправлен в Telegram."}, status=status.HTTP_200_OK)
...
class ReportToggleDailyView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"daily_pdf_enabled": settings_obj.daily_pdf_enabled}, status=status.HTTP_200_OK)
